...
class Models(enum.Enum):
    ...
    M6 = GridSearchCV(
        ...
        cv=CV, return_train_score=True, n_jobs=-1, verbose=10, scoring=scorer)
    """
...
def main():
    ...
    for comb in data_combs:
        ...
        for m in Models:
            file = '{}.csv'.format(m.name)
            gen_model_file = gen_model_data_folder.joinpath(file)
            selected_features_model_file = selected_features_model_data_folder.joinpath(file)
